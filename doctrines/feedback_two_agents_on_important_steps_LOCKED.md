...
metadata: 
  ...
  ratified_by: GPT (Agent Deployment Map) + the operator directive
  originSessionId: e3c6788c-6e96-4357-a8f9-25b72abf0e69
---

# Two agents on important steps · solo writes never · the PRIME RULE

the operator 2026-05-26 · *"you neeeed 2 agents on important steps !!!!!"*
...
| Any LOAD-BEARING doctrine lock | the wording binds future the agent behavior | the agent-writes + skeptic_loop judge (codex) before lock |
| Any LOAD-BEARING code edit | mutates production behavior | the agent-writes + skeptic_loop judge OR direct cross-vendor smoke-eval before apply |
...
**Operational ·** the pair pattern is implemented in `/root/.claude/skills/skeptic_loop/` (executor = the agent Opus 4.7 · judge = codex gpt-5.5 · target-score 5/5 · max 5 iterations). When skeptic_loop converges-broken (no score improvement across iterations), the spec is too complex for one pass · break it smaller · ship the minimal first brick · skeptic_loop the next.
